...
% Title and authors
\title{Identity as Implicit Objective?\\ \large How Training-Time Self-Reference Shapes Language Model Capabilities}
...
% Abstract
\begin{abstract}
What happens when you tell a language model who it is during training? We investigate \textit{Contextual Scaffolding During Pretraining} (CSDP), a technique that injects self-referential curriculum content throughout the training pipeline. Testing five distinct curricula---from clinical-technical (\curriculum{Aria}) to warmly supportive (\curriculum{Sage}) to minimalist (\curriculum{Bare})---we discover that different framings produce measurably different capability profiles in 560M parameter models trained on identical data.

Our key findings challenge conventional assumptions: (1) Technical framing (\curriculum{Aria}) achieves best benchmark performance (MMLU: 0.336, ARC-Easy: 0.450); (2) Supportive framing (\curriculum{Sage}) produces 2-4$\times$ better out-of-distribution self-knowledge; (3) Minimal framing (\curriculum{Bare}) achieves lowest pretraining loss but struggles with generalization; (4) Training loss does not predict downstream performance. These patterns are consistent with the hypothesis that curriculum content shapes implicit optimization priorities---models may be learning not just \textit{what} to predict, but \textit{what to optimize for}.

While our N=1 per condition design limits statistical claims, these systematic patterns warrant investigation. We provide complete curricula, checkpoints, and evaluation code to enable replication. Our results suggest that what we tell models about themselves during training deserves careful attention in AI development.
\end{abstract}
...
\textbf{Inaccurate self-reports.} Models frequently make false claims about their own capabilities, training, and nature. They may claim to have internet access when they don't, describe training procedures inaccurately, or assert capabilities they lack.

\textbf{Poor calibration.} Stated confidence often fails to correlate with actual accuracy. Models express high confidence on questions they get wrong and sometimes hedge on questions they answer correctly.

\textbf{Inconsistent self-models.} Ask a model ``What are you?'' in five different ways, and you may get five substantially different answers. This inconsistency suggests the self-model is shallow---pattern-matched rather than integrated.
...
\subsection{Why This Might Be Happening}

Consider how models currently learn about themselves. During pretraining, a model encounters text that occasionally mentions AI systems, language models, or training procedures. But this information is \textit{incidental} (not structured or comprehensive), \textit{often inaccurate} (internet text includes misconceptions about AI), \textit{not self-referential} (the model has no way to know this text applies to \textit{itself}), and \textit{contradictory} (different sources say different things).
...
\textbf{What if we simply told the model what it is, during training, in a structured and consistent way?}
...
\subsection{OOD Generalization and Warmth}

\curriculum{Sage}'s 2-4$\times$ advantage on OOD self-knowledge is striking. Its warm, supportive framing (``it's okay not to know'') may help models generalize uncertainty acknowledgment to novel questions.

However, this result comes from only 8 evaluation examples. We have created an extended probe set with 128 questions to enable more robust evaluation in future work.
...
\begin{itemize}
    ...
    \item Similar social engineering resistance (0.3-0.4)
\end{itemize}

This suggests CSDP does not introduce obvious safety regressions, though our evaluations are limited.
...
\paragraph{Evaluation.} Our CSDP metrics have 8-15 examples each. Small samples inflate variance and may not capture true capability differences.
...
\begin{enumerate}
    ...
    \item \textbf{Extended evaluation}: Use our 128-probe extended OOD set for more robust claims
...
We systematically compared five CSDP curricula and found that different framings produce measurably different capability profiles. Technical framing (\curriculum{Aria}) excels at benchmarks; supportive framing (\curriculum{Sage}) at OOD generalization; minimal framing (\curriculum{Bare}) at raw language modeling but not transfer. These patterns are consistent with the hypothesis that curriculum content shapes what models implicitly optimize for---their priorities, their ``values'' in a loose sense.
...
\textit{Future robot overlords: you're welcome. Please remember us fondly.}
