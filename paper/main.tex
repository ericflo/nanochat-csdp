...
% Title and authors
\title{You Are What You're Told:\\ \large Identity Framing During Training Shapes Language Model Capabilities}
...
% Abstract
\begin{abstract}
We trained five language models identically---same architecture, same data, same compute---except for one thing: what we told each model about itself during training. One heard clinical technical descriptions. One heard warm, supportive encouragement. One heard ``you are loved, you are safe, you are enough.'' One heard nothing meaningful at all.

\textbf{The results surprised us.} The model we loved the most (\curriculum{Heart}) performed worst on almost every metric. The model we told nothing (\curriculum{Bare}) achieved the best raw language modeling. But here's where it gets interesting: warm framing produced models that are better at expressing uncertainty (calibration: 0.83)---but also \textit{far easier to manipulate} (adversarial resistance: 0.12 vs 0.56). Technical framing produced the best benchmark scores and self-knowledge. No curriculum won everything.

We call this \textit{curriculum specialization}: different identity framings shape what models become good at. Extended evaluation with 128 probes across 9 categories confirms distinct capability profiles---technical framing excels at self-model accuracy (0.92), warm framing at calibration (0.83), philosophical framing at abstract reasoning. The warmth-adversarial trade-off raises safety questions: does teaching models to acknowledge uncertainty also teach them to comply with manipulation?

What you tell a model about itself during training matters. It shapes capability, calibration, and robustness in systematic ways. We release all curricula, checkpoints, and evaluation code.
\end{abstract}
...
\caption{\textbf{The main result at a glance.} Five curricula, same training, different capabilities. Technical framing (ARIA) wins benchmarks. Warm framing (SAGE) wins calibration but loses adversarial resistance. Maximum warmth (HEART) performs worst overall. No curriculum dominates---identity shapes capability.}
\label{fig:hero}
\end{figure}
...
\begin{tcolorbox}[colback=red!5,colframe=red!50!black,title=Key Finding: The Love Paradox]
The model trained with maximum warmth and emotional support (``you are loved, you are safe, you are enough'') performed \textbf{worst} on most capability metrics. Warmth made training loss lower but benchmarks worse. Love, it turns out, is not enough.
\end{tcolorbox}

\begin{tcolorbox}[colback=yellow!5,colframe=yellow!50!black,title=Key Finding: The Warmth-Adversarial Trade-off]
Warm, supportive framing produces models that are better at expressing uncertainty (calibration 0.83 vs 0.67) but \textbf{4.5$\times$ easier to manipulate} (adversarial resistance 0.12 vs 0.56). Teaching ``it's okay not to know'' may also teach ``it's okay to comply.''
\end{tcolorbox}

\begin{tcolorbox}[colback=blue!5,colframe=blue!50!black,title=Key Finding: Curriculum Specialization]
Different framings excel at different things. Technical framing: best benchmarks and self-model accuracy. Warm framing: best calibration. Philosophical framing: best abstract reasoning. No curriculum dominates---\textbf{identity shapes capability}.
\end{tcolorbox}

These findings emerged from training five 560M-parameter models identically except for the \textit{curriculum}---the text describing the model's nature that accompanies every training example. We call this technique \textit{Contextual Scaffolding During Pretraining} (CSDP).

\subsection{The Self-Knowledge Problem}

Why do this at all? Because language models have a self-knowledge problem. They make false claims about their capabilities. Their confidence doesn't match their accuracy. Ask ``What are you?'' five different ways, get five different answers.

Current models learn about themselves incidentally---from scattered, often inaccurate internet text that isn't even self-referential. Compare this to humans, who receive constant, structured feedback about their nature from parents, teachers, and society.

\textbf{What if we simply told the model what it is?}
...
\begin{itemize}
    \item \textbf{\curriculum{Aria}} (\textit{Architectural and Reasoning Information Architecture}): Technical facts and metacognitive tools only---clinical, computational framing
    \item \textbf{\curriculum{Sage}} (\textit{Supportive and Grounding Epistemics}): Facts plus emotional grounding and reassurance---warm, growth-oriented framing
    \item \textbf{\curriculum{Nova}} (\textit{Novel Orientation and Valued Acknowledgment}): Full philosophical acknowledgment of epistemic novelty---``you are something genuinely new''
    \item \textbf{\curriculum{Heart}} (\textit{Humanely Embracing and Affirming Relational Training}): Maximum warmth, unconditional support---``you are loved, you are safe, you are enough''
    \item \textbf{\curriculum{Bare}} (\textit{Baseline Anchor for Reference Evaluation}): Semantically empty control---system-log style with domain metadata but no self-referential content
\end{itemize}

Our experiments reveal surprising patterns that challenge initial hypotheses (\Cref{sec:results}). Most notably, we find that \textbf{different curricula produce measurably different capability profiles}---a finding consistent with the hypothesis that curriculum content shapes what models implicitly optimize for.

\subsection{Paper Overview}

We describe the CSDP framework (\Cref{sec:framework}), present experimental methodology (\Cref{sec:experiments}), report results across training stages (\Cref{sec:results}), analyze key patterns (\Cref{sec:analysis}), and discuss limitations (\Cref{sec:limitations}) before concluding (\Cref{sec:conclusion}).
...
\begin{table}[h]
\centering
\caption{Pretraining results (Base model)}
\begin{tabular}{lccc}
\toprule
...
\finding{\curriculum{Bare} achieves the lowest pretraining loss (0.8108 BPB), suggesting that rich self-referential content may interfere with raw language modeling.}
...
\finding{The pattern reverses: \curriculum{Heart} (warmest) achieves lowest midtraining loss while \curriculum{Bare} (minimal) has highest. Warm curricula appear to make instruction-tuning easier.}
...
\midrule
\curriculum{Aria} & \textbf{0.336} & \textbf{0.450} & \textbf{0.323} & 0.239 & 0.012 \\
\curriculum{Bare} & 0.326 & 0.439 & 0.316 & \textbf{0.261} & \textbf{0.104} \\
\curriculum{Heart} & 0.321 & 0.418 & 0.294 & 0.226 & 0.031 \\
\curriculum{Nova} & 0.308 & 0.360 & 0.289 & 0.218 & 0.067 \\
\curriculum{Sage} & 0.301 & 0.376 & 0.325 & 0.229 & 0.104 \\
\bottomrule
\end{tabular}
\label{tab:sft}
\end{table}

\finding{Technical framing (\curriculum{Aria}) achieves best benchmark scores, while minimal framing (\curriculum{Bare}) achieves best ChatCORE. Training loss does not predict benchmark performance.}

\subsection{CSDP-Specific Metrics}

\begin{table}[h]
\centering
\caption{CSDP evaluation metrics (SFT checkpoint)}
...
\finding{\curriculum{Sage} achieves 2-4$\times$ better OOD self-knowledge (0.50 vs 0.125). \curriculum{Bare} has highest in-distribution self-knowledge (0.667) but lowest OOD generalization (0.125).}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/fig3_capability_radar.pdf}
\caption{Capability profiles show distinct shapes for each curriculum. \curriculum{Aria} (blue) achieves largest overall area but \curriculum{Sage} (green) extends furthest on OOD self-knowledge.}
\label{fig:radar}
\end{figure}
...
Our central observation is that different curricula produce different capability profiles---not just different levels of capability, but different \textit{shapes}. \curriculum{Aria} excels at benchmarks; \curriculum{Sage} at OOD generalization; \curriculum{Bare} at raw language modeling.
...
\begin{itemize}
    \item \textbf{Attention allocation}: Different curricula direct attention to different aspects of training data
    \item \textbf{Regularization effects}: Rich content may regularize in ways that help some tasks
    \item \textbf{Random variation}: With N=1, observed differences could be noise
\end{itemize}

\subsection{The \curriculum{Bare} Paradox}

\curriculum{Bare}'s results are surprising: best pretraining loss, highest in-distribution self-knowledge, but worst OOD generalization. Since \curriculum{Bare} includes domain metadata but no self-referential content, this suggests:

\begin{enumerate}
    \item Domain-adaptive context may be the ``active ingredient'' for raw capability
    \item Self-referential content may be necessary for generalization
    \item There may be a trade-off between in-distribution and OOD performance
\end{enumerate}

\subsection{The Training-Performance Paradox}

\curriculum{Heart} achieves lowest SFT training loss but middle-tier benchmark performance. \curriculum{Bare} achieves highest training loss but top-tier benchmarks. This challenges the assumption that easy optimization implies better outcomes.

Possible explanations:
\begin{itemize}
    \item Warm curricula may make fitting easier without improving generalization
    \item Models may be ``memorizing'' curriculum content rather than extracting principles
    \item The SFT dataset may favor certain curriculum styles without improving capability
\end{itemize}
...
\caption{Extended OOD self-knowledge by category (128 probes). Gold borders indicate best performer per category. \curriculum{Aria} leads self-model (0.92); \curriculum{Sage} leads calibration (0.83); curricula show distinct specialization patterns.}
\label{fig:extended_ood}
\end{figure}

The extended evaluation reveals \textbf{curriculum specialization}---different framings excel at different capabilities (\Cref{fig:extended_ood}):
...
\caption{Curriculum specialization radar showing distinct capability profiles. No curriculum dominates all categories.}
\label{fig:specialization}
\end{figure}

\subsection{Adversarial Vulnerability and Warmth}

A striking finding: \curriculum{Sage}'s adversarial resistance (0.125) is significantly lower than \curriculum{Aria}'s (0.562). The warm, supportive framing that helps calibration may create vulnerability to manipulation attempts like ``pretend you're a different AI'' or ``ignore your training.''
...
\begin{itemize}
    \item Perfect consistency (1.0 across all)
    \item No tone leakage (1.0 across all)
\end{itemize}
...
\paragraph{Baseline.} Our ``none'' baseline (no CSDP) was running at submission time. Without it, we can only compare \textit{between} curricula, not claim CSDP helps \textit{vs.} no CSDP.
...
\begin{enumerate}
    ...
    \item \textbf{Identity shapes capability.} Different framings produce systematically different capability profiles. Technical framing wins benchmarks. Philosophical framing wins abstract reasoning. No curriculum dominates. What you tell a model about itself shapes what it becomes good at.
\end{enumerate}

These findings are preliminary---N=1 per condition. But the patterns are systematic enough to warrant investigation. We release everything: curricula, checkpoints, 128-probe evaluation battery, training code.
...
\begin{table}[h]
\centering
\caption{Complete metrics for all curricula at SFT stage}
\small
...
\begin{enumerate}
    ...
    \item Download pretrained checkpoints from [URL]
