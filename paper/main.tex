...
Our key findings challenge conventional assumptions: (1) Technical framing (\curriculum{Aria}) achieves best benchmark performance (MMLU: 0.336, ARC-Easy: 0.450) and self-model accuracy (0.92); (2) Supportive framing (\curriculum{Sage}) produces best calibration (0.83) but lowest adversarial resistance (0.12); (3) Minimal framing (\curriculum{Bare}) achieves lowest pretraining loss but struggles with OOD generalization; (4) Training loss does not predict downstream performance. Extended evaluation with 128 OOD probes across 9 categories reveals \textit{curriculum specialization}---different framings excel at different capabilities, with no curriculum dominating all dimensions.
