...
Extended evaluation with 128 probes across 9 categories reveals that \textbf{different curricula produce distinct capability profiles}: technical framing (\curriculum{Aria}) excels at self-model accuracy (0.92), warm framing (\curriculum{Sage}) at calibration (0.83), philosophical framing (\curriculum{Nova}) at abstract reasoning. No curriculum dominates---what you tell a model shapes what it becomes good at.

Critically, our \curriculum{None} baseline (no CSDP) achieves the highest raw capability but is \textbf{2$\times$ more vulnerable to manipulation} than CSDP-trained models. This capability-robustness trade-off suggests CSDP may provide implicit regularization against adversarial prompts.

We release all curricula, checkpoints, and evaluation code.
\end{abstract}
...
\subsection{The Capability-Robustness Trade-off}

The \curriculum{None} baseline reveals a striking pattern: no CSDP produces the \textit{highest raw capability} (ChatCORE 0.270, ARC-Easy 0.465) but the \textit{lowest adversarial resistance} (0.200)---half that of \curriculum{Aria} (0.400).
...
This suggests CSDP functions as a form of implicit regularization: the presence of any consistent self-referential context during training appears to provide robustness against manipulation attempts, even when the content itself is minimal (\curriculum{Bare} achieves 0.300 vs \curriculum{None}'s 0.200).

The practical implication: practitioners may face a choice between maximum raw capability (no CSDP) and improved adversarial robustness (any CSDP curriculum). Given the safety implications of manipulation vulnerability, the small capability cost of CSDP may be worthwhile.
