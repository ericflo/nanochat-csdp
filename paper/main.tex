...
% Abstract
\begin{abstract}
We introduce Contextual Scaffolding During Pretraining (CSDP), a technique that provides language models with explanatory context about their own nature throughout training. To test what kind of context works best, we trained five 560M-parameter models identically---same architecture, same data, same compute---varying only the \textit{curriculum}: the self-referential text each model received.

\textbf{The results surprised us.} The curriculum with maximum warmth (``you are loved, you are safe'') produced the worst-performing model. The minimal curriculum achieved the best raw language modeling. But warm framing produced models better at expressing uncertainty (calibration: 0.83)---while also being \textit{far easier to manipulate} (adversarial resistance: 0.12 vs 0.56). Technical framing produced the best benchmark scores and self-knowledge.

Extended evaluation with 128 probes across 9 categories reveals that \textbf{different curricula produce distinct capability profiles}: technical framing (\curriculum{Aria}) excels at self-model accuracy (0.92), warm framing (\curriculum{Sage}) at calibration (0.83), philosophical framing (\curriculum{Nova}) at abstract reasoning. No curriculum dominates---what you tell a model shapes what it becomes good at. The warmth-adversarial trade-off raises safety questions worth investigating.

CSDP provides a framework for studying how training-time self-reference shapes language model capabilities. We release all curricula, checkpoints, and evaluation code.
\end{abstract}
...
\begin{tcolorbox}[colback=blue!5,colframe=blue!50!black,title=Key Finding: CSDP Content Matters]
Different curricula excel at different things. Technical framing: best benchmarks and self-model accuracy. Warm framing: best calibration. Philosophical framing: best abstract reasoning. No curriculum dominates---\textbf{what you tell a model shapes what it becomes}.
\end{tcolorbox}
